...
**Money/landing pages must lead the `title` prop with the page's primary target keyword.** `SEO.astro` only appends `SITE.name` (`pageTitle = \`${title}  ${SITE.name}\``), so the per-page `title` prop is the *only* place the target keyword lands in the `<title>` tag. A generic title (e.g. `บริการของเรา`) silently drops the keyword from `<title>` — a major on-page penalty (SEO audits score this −3). Mirror the same keyword in the meta `description` and a real on-page `<h1>`, and verify it in the **rendered** `dist/**/index.html`, not the source. Example: `/services` targets `โรงงานผลิตเสื้อผ้า` → its `title` leads with that phrase.

Site-wide constants (name, locale `th_TH`, social, default OG image) live in `src/consts.ts` (`SITE`). RSS (`src/pages/rss.xml.ts`), sitemap, and robots.txt are all driven from this + `astro.config.mjs`. `SITE_URL` in `astro.config.mjs` (read from `.env`, currently defaulting to the punycode domain `https://xn--o3c1bj3b4bj8cd.com/`) controls sitemap, robots, RSS, and canonical URLs — keep it pointed at the real production domain. Note: `robots.txt` is generated into `dist/` at build (`astro-robots-txt`); a `502`/`404` for `/robots.txt` on the live site is a **hosting/deploy** issue, not a code bug — confirm the file exists in `dist/` first.
