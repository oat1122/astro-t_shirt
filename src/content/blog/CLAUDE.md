...
- **`tag` ห้ามตั้งชื่อชนกับ `category`** (เช่น category "งานสกรีน" + tag "งานสกรีน") — จะได้ 2 หน้า
  ที่มีบทความชุดเดียวกัน = near-duplicate แข่งกันเอง ให้เลือกใช้คำนั้นเป็น category **หรือ** tag อย่างใดอย่างหนึ่ง
- หน้า `/tag/<ชื่อ>` ตั้ง `noindex` ไว้แล้ว (กัน thin/duplicate กิน crawl budget) — ใช้ navigate บนเว็บได้
  แต่ไม่เข้าดัชนี Google ส่วนหน้า `/category/<ชื่อ>` ยัง index ปกติ ∴ จัดบทความลง category ให้ดี
  ไม่ต้องหวังให้หน้า tag ติดอันดับ (อย่าเขียนบทความเพื่อปั้นหน้า tag)
...
- [ ] `category` + `tags` สะกดตรงกับที่ใช้อยู่เดิม (ไม่สร้าง duplicate) — และ `tag` ไม่ตั้งชื่อชนกับ `category` (§3)
